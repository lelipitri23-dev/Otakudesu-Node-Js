...
app.get('/admin', isAdmin, async (req, res) => {
  try {
    const [totalAnime,
      totalEpisodes,
      totalUsers,
      totalComments] = await Promise.all([
...
app.post('/admin/anime/add', isAdmin, upload.single('animeImage'), async (req, res) => {
  try {
    ...
    if (await Anime.findOne({
      pageSlug: formData.pageSlug
    })) return res.status(400).send('Slug sudah ada.');
...
app.get('/admin/anime/:slug/edit', isAdmin, async (req, res) => {
  try {
    const anime = await Anime.findOne({
      pageSlug: decodeURIComponent(req.params.slug)
    }).lean();
...
app.post('/admin/anime/:slug/episodes/add', isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (await Episode.findOne({
      episodeSlug
    })) return res.status(400).send('Slug Episode sudah ada.');
    const parentAnime = await Anime.findOne({
    ...
    await Anime.updateOne({
      ...
      $push: {
        episodes: {
          title: episodeTitle, url: episodeSlug, date: episodeDate || new Date().toLocaleDateString('id-ID'), _id: createdEpisode._id
        }
      }
    });
...
app.get('/admin/episode/:slug(*)/edit', isAdmin, async (req, res) => {
  try {
    const episode = await Episode.findOne({
      episodeSlug: "/" + decodeURIComponent(req.params.slug)
    }).lean();
...
app.post('/admin/episode/:slug(*)/delete', isAdmin, async (req, res) => {
  try {
    ...
    if (episode) await Comment.deleteMany({
      episode: episode._id
    });
    res.redirect('/admin/episodes');
  } catch (error) {
